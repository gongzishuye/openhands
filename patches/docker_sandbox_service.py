...
class DockerSandboxService(SandboxService):
    ...
    async def _container_to_sandbox_info(self, container) -> SandboxInfo | None:
        ...
        # OH multi-user patch: recover the owning user from the container
        # label stamped at creation (see start_sandbox). None for legacy
        # containers created before this patch.
        user_label = (container.labels or {}).get('oh_user_id') or None
        return SandboxInfo(
            id=container.name,
            created_by_user_id=user_label,
    ...
    async def start_sandbox(
        ...
        created_by_user_id: str | None = None,  # OH multi-user patch
    ) -> SandboxInfo:
        """Start a new sandbox."""
        ...
        labels = {
            ...
            # OH multi-user patch: stamp the owning user so the sandbox can be
            # matched back to its user (read in _container_to_sandbox_info).
            'oh_user_id': created_by_user_id or '',
        }
...
class DockerSandboxServiceInjector(SandboxServiceInjector):
    ...
    max_num_sandboxes: int = Field(
        # OH multi-user patch: env-configurable (Injector doesn't bind env).
