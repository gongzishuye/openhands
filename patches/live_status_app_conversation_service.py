...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    async def _get_current_user_sandbox_ids(self) -> list[str] | None:
        """OH multi-user patch: sandbox ids owned by the current user, to scope
        the conversation list (conversations have no user_id column; their only
        link to a user is via their sandbox). Returns None when there is no user
        id (single-user / unauthenticated) so no filtering is applied."""
        try:
        ...
        while True:
            ...
            for sandbox in page.items:
                if sandbox.created_by_user_id == user_id:
    ...
    async def _filter_owned(
        self, conversations: list  # list[AppConversation | None]
    ) -> list:
        """OH multi-user patch: enforce per-user ownership on direct fetches.
        Any conversation whose sandbox is not owned by the current user is
        replaced with None (callers already handle None). No-op when there is
        no user id (single-user / unauthenticated)."""
        sandbox_ids = await self._get_current_user_sandbox_ids()
        if sandbox_ids is None:
            return conversations
        allowed = set(sandbox_ids)
        return [
            c if (c is not None and c.sandbox_id in allowed) else None
    ...
    async def search_app_conversations(
        ...
        # OH multi-user patch: restrict to the current user's sandboxes.
    ...
    async def get_app_conversation(
        ...
        # OH multi-user patch: enforce ownership (prevents cross-user read by id)
        result = await self._filter_owned(result)
        return result[0]
    ...
    async def batch_get_app_conversations(
        ...
        # OH multi-user patch: enforce ownership (prevents cross-user read by id)
        conversations = await self._filter_owned(conversations)
        return conversations
        ...
        if not task.request.sandbox_id:
            ...
            if sandbox is None:
                ...
                sandbox = await self.sandbox_service.start_sandbox(
                    ...
                    created_by_user_id=task.created_by_user_id,  # OH multi-user
                )
    ...
    async def update_app_conversation(
        ...
        # OH multi-user patch: enforce ownership (prevents cross-user update by id)
        owned = await self._get_current_user_sandbox_ids()
        if owned is not None and info.sandbox_id not in set(owned):
            return None
