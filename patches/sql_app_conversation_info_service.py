...
class SQLAppConversationInfoService(AppConversationInfoService):
    ...
    async def search_app_conversation_info(
        ...
        sandbox_id__in: list[str] | None = None,  # OH multi-user patch
    ...
    def _apply_filters(
        ...
        # OH multi-user patch: restrict to a set of sandbox ids (a user's
        # sandboxes). Empty list => match nothing (avoid invalid ``IN ()``).
        if sandbox_id__in is not None:
            if sandbox_id__in:
                conditions.append(
                    StoredConversationMetadata.sandbox_id.in_(sandbox_id__in)
                )
            else:
