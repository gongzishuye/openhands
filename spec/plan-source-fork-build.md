...
> 模式：fork + 长期分支 `multiuser-deploy`；**在一台大内存机器（≥8G）手动构建并 push**；3.5G 生产机只 pull + run。
...
- ⚠️ 生产机 3.5G **不可** build 前端（CON-002）→ 构建只在大内存机器（≥8G）。
...
| T4 | 大内存机器构建 + push 镜像 | `ghcr.io/gongzishuye/openhands-custom:96eea1b-mu1` | T2, T3 |
...
### T4 — 构建 + push（在大内存机器 ≥8G，**不在生产机**）
在一台内存 ≥8G、装好 Docker 的机器上拉取 fork 的 `multiuser-deploy` 分支后：
```bash
# 1) 登录镜像仓库（GHCR 用一个有 write:packages 权限的 token；勿明文入库）
echo "<GHCR_TOKEN>" | docker login ghcr.io -u gongzishuye --password-stdin
# 2) 构建（前端 build 在此机完成，生产机不参与）
...
```
- 构建机可以是云上临时开的 8G+ 实例、或本地 PC；构建完即可释放，只要镜像已 push。
- 这台机器与生产机分离：生产机全程只 `docker pull` + `run`，不碰构建。
...
- **R2 前端构建 OOM**：违反 CON-002——必须在大内存机器（≥8G）；**绝不**在 3.5G 生产机 build。
- **R3 补丁 apply reject**：基线非 `96eea1b`；核对 commit 重试。
- **R4 切换窗口**：停旧起新之间 :3000 秒级不可用，可接受（R 同 plan-frontend §R3）。
- **R5 token 泄漏**：`docker login` / `git push` 的 token 勿明文入库/聊天；构建机上用完即 `docker logout` 并吊销。
...
- [ ] T4：大内存机器构建成功，镜像 push 且生产机可 pull
